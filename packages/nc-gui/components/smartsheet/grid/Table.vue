...
const { isPkAvail, isSqlView, eventBus, allFilters, validFiltersFromUrlParams, sorts, isViewOperationsAllowed } =
...
type ReloadViewDataParams = void | {
  shouldShowLoading?: boolean | undefined
  offset?: number | undefined
  isFromLinkRecord?: boolean | undefined
...
async function refreshCachedLinkRecordRow(rowId?: string) {
  await refreshCachedRow(rowId, {
    findRow: (targetRowId) => {
      ...
      return rowIndex === -1 || !cachedRow ? undefined : [rowIndex, cachedRow]
    },
    setRow: (rowIndex, row) => {
      dataRef.value[rowIndex] = row
    },
  })
}
