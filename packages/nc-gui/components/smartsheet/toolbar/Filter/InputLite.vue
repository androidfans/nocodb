...
import { ClientType, SqlUiFactory, UITypes, isLinksOrLTAR } from 'nocodb-sdk'
...
type FilterUIType = keyof typeof checkTypeFunctions
...
const componentMap: Partial<Record<FilterUIType, any>> = computed(() => {
  return {
    ...
    isInt: Integer,
    isFloat: Float,
    // Links & LTAR share this slot — see FilterInput.vue for full explanation
...
const isSingleOrMultiSelect = computed(() => {
  return filterUIType.value === 'isSingleSelect' || filterUIType.value === 'isMultiSelect'
})
</script>
...
<template>
  ...
  >
    <component
      ...
      :class="{
        'text-nc-content-gray-muted pointer-events-none': disabled,
      }"
